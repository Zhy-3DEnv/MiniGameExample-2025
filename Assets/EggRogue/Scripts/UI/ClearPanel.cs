...
/// <summary>
/// 完整通关结算界面 - 通过最后一关后显示，可查看到达关卡、当前金币等，选择返回主菜单或再玩一次。
/// 使用方式：
/// 1. 在 Canvas 下创建 Panel，命名为 ClearPanel
/// 2. 添加本脚本
/// 3. 在 Inspector 中绑定 UI 元素（标题、关卡/金币文本、返回主菜单 / 再玩一次按钮）
/// </summary>
public class ClearPanel : BaseUIPanel
{
    [Header("UI 引用")]
    [Tooltip("标题文本（例如：恭喜通关）")]
    ...
    /// <summary>
    /// 显示完整通关结算界面（外部调用，例如 LevelManager 通关或 ResultPanel 继续时）。
    /// </summary>
    ...
    protected override void OnShow()
    {
        ...
        if (EggRogue.GameplayPauseManager.Instance != null)
            EggRogue.GameplayPauseManager.Instance.RequestPause("ClearPanel");
    }
    ...
    protected override void OnHide()
    {
        base.OnHide();
        if (EggRogue.GameplayPauseManager.Instance != null)
            EggRogue.GameplayPauseManager.Instance.RequestResume("ClearPanel");
    }
}
